    ...
    <div className="lms-stack">
      <div className="lms-between lms-wrap">
        ...
        {isTeacher && (
          <div className="lms-flex lms-gap-sm">
            <Link href={`/lms/courses/${course.id}/assignments/new-quiz`} className="lms-btn lms-btn--sm">
              + Quiz
            </Link>
            <Link href={`/lms/courses/${course.id}/assignments/new`} className="lms-btn lms-btn--primary lms-btn--sm">
              + Assignment
            </Link>
          </div>
        )}
      ...
      {assignments.length === 0 ? (
        ...
        <div className="lms-list">
          {assignments.map((a) => (
            <HoverLink key={a.id} href={`/lms/courses/${course.id}/assignments/${a.id}`} className="lms-row">
