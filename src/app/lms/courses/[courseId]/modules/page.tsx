...
function ModuleItemRow({ item, courseId }: { item: ModuleItem; courseId: string }) {
  ...
  if (item.kind === 'assignment' && item.refId) {
    return (
      <HoverLink href={`/lms/courses/${courseId}/assignments/${item.refId}`} className="lms-modlink">
        {inner}
      </HoverLink>
    )
  }
  if (item.kind === 'page' && item.refId) {
    return (
      <HoverLink href={`/lms/courses/${courseId}/pages/${item.refId}`} className="lms-modlink">
        {inner}
      </HoverLink>
    )
  }
