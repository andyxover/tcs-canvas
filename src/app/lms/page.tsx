    ...
    <section className="lms-card lms-card--pad">
      ...
      {total === 0 ? (
        ...
        <div className="lms-stack" style={{ gap: 8, marginTop: 10 }}>
          {withWork.map((e) => (
            <HoverLink key={e.courseId} href={`/lms/courses/${e.courseId}/assignments`} className="lms-hub-row">
  ...
  return (
    <section className="lms-card lms-card--pad">
      <div className="lms-between" style={{ marginBottom: 10 }}>
        ...
        <Link href="/lms/agenda" className="lms-btn lms-btn--ghost lms-btn--sm">
          Full agenda →
        </Link>
      </div>
      ...
      {upcoming.length === 0 ? (
        ...
        <div className="lms-stack" style={{ gap: 8 }}>
          {upcoming.map((x) => (
            <HoverLink key={x.assignmentId} href={`/lms/courses/${x.courseId}/assignments/${x.assignmentId}`} className="lms-hub-row">
...
async function TeacherCourseCard({ course }: { course: Course }) {
  ...
  return (
    <Link href={`/lms/courses/${course.id}`} className="lms-minicard">
