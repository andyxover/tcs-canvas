...
export default async function SearchPage({ searchParams }: { searchParams: Promise<{ q?: string }> }) {
  ...
  const results = await search(q, viewer.kind, viewer.person.id)
      ...
      {results.courses.length > 0 && (
        <Section title="Courses">
          {results.courses.map((c) => (
            <HoverLink key={c.id} href={`/lms/courses/${c.id}`} className="lms-hub-row">
      ...
      {results.assignments.length > 0 && (
        <Section title="Assignments">
          {results.assignments.map((a) => (
            <HoverLink key={a.id} href={`/lms/courses/${a.courseId}/assignments/${a.id}`} className="lms-hub-row">
